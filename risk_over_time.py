...
years = st.number_input(label='Time (truncates to integer)', min_value=0, value=10)
...
sigma = st.number_input(label=unicodeit.replace('\sigma^*'), \
                        min_value=0., value=0., format='%.15g')
...
# FIXME review recalc() func, likely need to run it here
